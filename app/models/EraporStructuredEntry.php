...
/** Internal BING/PPI autosave. No public route.
 * Server supplies authenticated actor and optional test clock.
 * Changes: key, value, expected. BING keys nilai:<id>/komentar:<id>;
 * PPI keys <aspek_id>:<kolom_id>. All keys are matched to DB definitions.
 */
final class EraporStructuredEntry
{
    public static function save(PDO $db,string $type,int $sessionId,int $documentId,int $actorId,array $changes,?DateTimeImmutable $clock=null): array
    {
        ...
        if (!in_array($type,['BING','PPI'],true) || min($sessionId,$documentId,$actorId)<1 || !$changes || count($changes)>30) throw new DomainException('Permintaan isian tidak valid.');
        $seen=[];
        ...
        try {
            ...
            if ($type==='PPI' && $session['kondisi']!=='ABK') throw new DomainException('PPI hanya tersedia untuk sesi ABK.');
            $doc=self::one($db,"SELECT d.rubrik_id FROM erapor_sesi_dokumen sd JOIN erapor_dokumen d ON d.id=sd.dokumen_id
                ...
                AND d.semester=? AND r.khusus_abk=? FOR UPDATE",[$sessionId,$documentId,$type,$session['semester'],$type==='PPI'?1:0]);
            if (!$doc) throw new DomainException('Dokumen tidak tersedia dalam sesi.');
            $rubricId=(int)$doc['rubrik_id'];
            $fields=self::fields($db,$type,$rubricId);
            $q=$db->prepare('SELECT kode FROM erapor_bing_skala WHERE rubrik_id=?'); $q->execute([$rubricId]);
            $grades=$q->fetchAll(PDO::FETCH_COLUMN); $updated=0;
            foreach ($changes as $change) {
                $field=$fields[$change['key']] ?? null;
                if (!$field) throw new DomainException('Isian bukan milik rubrik/sesi; Hasil Capaian tidak diisi di sini.');
                ...
                if ($field['column']==='isi') {
                    ...
                    foreach ([$value,$expected] as $grade) if ($grade!==null && !in_array($grade,$grades,true)) throw new DomainException('Skala BING tidak valid.');
                }
                ...
                $old=self::one($db,'SELECT '.$field['column'].' AS value FROM '.$field['table'].' WHERE '.$where.' FOR UPDATE',array_values($keys));
                ...
                if ($value===null) {
                    ...
                    $db->prepare('UPDATE '.$field['table'].' SET '.$field['column'].'=?,diisi_oleh=?,diisi_pada=CURRENT_TIMESTAMP WHERE '.$where)->execute([$value,$actorId,...array_values($keys)]);
                } else {
                    $row=$keys+[$field['column']=>$value,'diisi_oleh'=>$actorId];
            ...
            foreach ($fields as $field) {
                ...
                $keys=['sesi_id'=>$sessionId,'dokumen_id'=>$documentId,'rubrik_id'=>$rubricId]+$field['ids'];
                $where=implode(' AND ',array_map(fn($k)=>$k.'=?',array_keys($keys)));
                $row=self::one($db,'SELECT '.$field['column'].' AS value FROM '.$field['table'].' WHERE '.$where,array_values($keys));
    ...
    private static function fields(PDO $db,string $type,int $rubricId): array
    {
        $fields=[];
        if ($type==='BING') {
            foreach ([
                ['erapor_bing_indikator','nilai','erapor_bing_nilai','pilihan_kode','indikator_id'],
                ['erapor_bing_komentar','komentar','erapor_bing_isian','isi','komentar_id'],
            ] as [$definition,$prefix,$table,$column,$idColumn]) {
                $q=$db->prepare('SELECT id,wajib FROM '.$definition.' WHERE rubrik_id=? ORDER BY urutan'); $q->execute([$rubricId]);
                foreach ($q->fetchAll(PDO::FETCH_ASSOC) as $row) $fields[$prefix.':'.$row['id']]=[
                    'table'=>$table,'column'=>$column,'ids'=>[$idColumn=>(int)$row['id']],'required'=>(bool)$row['wajib']];
            }
        } else {
            $q=$db->prepare('SELECT a.id AS aspek_id,c.id AS kolom_id,c.wajib FROM erapor_ppi_aspek a JOIN erapor_ppi_kolom c ON c.rubrik_id=a.rubrik_id WHERE a.rubrik_id=? AND a.aktif=1 AND c.diisi_di_sesi=1 ORDER BY a.urutan,c.bagian,c.urutan'); $q->execute([$rubricId]);
            foreach ($q->fetchAll(PDO::FETCH_ASSOC) as $row) $fields[$row['aspek_id'].':'.$row['kolom_id']]=[
                'table'=>'erapor_ppi_isian','column'=>'isi','ids'=>['aspek_id'=>(int)$row['aspek_id'],'kolom_id'=>(int)$row['kolom_id']],'required'=>(bool)$row['wajib']];
        }
