...
`tests/erapor-api-adapter.php`: 25 skenario controller JSON/RBAC/CSRF di subprocess CLI dengan SQLite terisolasi dan service spies. `tests/erapor-catalog-mysql.php --run --backup=<backup.sql>` juga menjalankan loopback HTTP nyata terhadap database restore disposable: dashboard read-only, sesi dibuat melalui POST, pembukaan ID sesi baru, login, GET sesi, autosave RTS/Agama/BING/PPI/Ummi, inisialisasi Ummi eksplisit, pencatatan/penghapusan tes dinamis, completion otoritatif, CSRF sekali pakai, dokumen asing, payload invalid, transisi salah, retry penerimaan terotorisasi, kepemilikan guru lain dan pencabutan RBAC termasuk penyembunyian aksi edit di dashboard. Harness berakhir dengan pemeriksaan checksum semua tabel legacy. `tests/erapor-session-ui.php` memeriksa markup untuk status BELUM_DIISI, TELAH_DIISI, dan MENUNGGU_TTD. `tests/route-rbac-regression.php`: 59 private routes ditolak lewat router/middleware nyata bila role tanpa izin. Regresi akun, portal guru dan workflow legacy lulus.
