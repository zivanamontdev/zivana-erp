...
Dashboard baru hanya mengeluarkan tombol Isi Rapor jika kalender valid dan semua rubrik paket siap. RAS yang belum tersedia memblokir paket AKHIR tanpa membuat data parsial. Saat ini halaman sesi hanya ringkasan data/paket dan progress; editor interaktif belum selesai, sehingga flag harus tetap mati pada production.
...
`tests/erapor-api-adapter.php`: 25 skenario controller JSON/RBAC/CSRF di subprocess CLI dengan SQLite terisolasi dan service spies. `tests/erapor-catalog-mysql.php --run --backup=<backup.sql>` juga menjalankan loopback HTTP nyata terhadap database restore disposable: dashboard read-only, sesi dibuat melalui POST, pembukaan ID sesi baru, login, GET sesi, autosave BING, CSRF sekali pakai, dokumen asing, payload invalid, transisi salah, kepemilikan guru lain dan pencabutan RBAC termasuk penyembunyian aksi edit di dashboard. Harness berakhir dengan pemeriksaan checksum semua tabel legacy. Total 1108 pemeriksaan MySQL. `tests/route-rbac-regression.php`: 59 private routes ditolak lewat router/middleware nyata bila role tanpa izin. Regresi akun, portal guru dan workflow legacy lulus.
