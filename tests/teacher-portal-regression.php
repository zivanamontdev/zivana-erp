...
CREATE TABLE rapor(id INTEGER PRIMARY KEY,murid_id INTEGER,guru_id INTEGER,sesi_pembagian_id INTEGER,template_id INTEGER,status TEXT,diarsipkan_at TEXT NULL);
...
INSERT INTO rapor VALUES(1,1,7,1,1,'belum_diisi',NULL),(2,1,7,2,1,'disetujui',NULL),(3,1,7,3,1,'belum_diisi',NULL),(4,2,8,4,1,'belum_diisi',NULL);
...
foreach([1=>'ganjil',2=>'genap',3=>'ganjil',4=>'genap'] as $period=>$semester){
    $id=10+$period;$db->exec("INSERT INTO rapor VALUES($id,1,7,$period,1,'belum_diisi',NULL)");
...
$db->exec("INSERT INTO rapor VALUES(20,1,7,1,99,'belum_diisi',NULL)");
invalidEntry(fn()=>ReportEntry::save(20,7,[],[],true));
portalCheck($db->exec("INSERT INTO murid VALUES(3,'Murid Arsip','789',1)")!==false,'Archive fixture student');
$db->exec('INSERT INTO kelas_guru_murid VALUES(3,7)');
$db->exec("INSERT INTO rapor VALUES(21,3,7,1,1,'belum_diisi',NULL)");
$_SESSION=['karyawan_id'=>7,'display_name'=>'Guru Uji','csrf_token'=>'fixture'];
$archiveController=new PengisianRaporController();
$archiveController->inputs=['nilai'=>[1=>2],'catatan'=>[1=>'Draft tersimpan']];
$archiveController->arsipkan('21');
portalCheck($archiveController->redirected==='/portal-guru/dashboard?sesi_id=1','Archive action returns to the same dashboard period');
portalCheck($db->query('SELECT diarsipkan_at FROM rapor WHERE id=21')->fetchColumn()!==null,'Archive action persists draft archive marker');
$archivedDashboard=TeacherPortal::dashboard(7,1,'2026-09-23');
$archivedStudent=array_values(array_filter($archivedDashboard['daftarMurid'],fn($student)=>(int)$student['rapor_id']===21))[0]??null;
portalCheck($archivedStudent!==null && $archivedStudent['rapor_diarsipkan_at']!==null,'Dashboard reads the archived draft marker');
ReportEntry::save(21,7,[1=>2,2=>1],[1=>'Final'],true);
portalCheck($db->query('SELECT diarsipkan_at FROM rapor WHERE id=21')->fetchColumn()===null,'Submitted report no longer carries draft archive state');
$_SESSION=['karyawan_id'=>7,'display_name'=>'Guru Uji','csrf_token'=>'fixture'];
...
portalCheck(str_contains($html,'Diarsipkan') && str_contains($html,'Lanjutkan') && str_contains($html,'/portal-guru/rapor/21'),'Archived report gets its badge and resume action');
...
portalCheck($xpath->query('//select[@data-report-switcher]')->length===0,'Student identity is text, not a dropdown');
portalCheck(str_contains($xpath->query('//p[@class="pengisian-student-name"]')->item(0)->textContent,$rapor['nama_lengkap']),'Header shows actual pupil name');
portalCheck($xpath->query('//div[@class="pengisian-header-top"]//button[@type="submit"]')->length===2,'Save and submit actions share the heading row');
portalCheck($xpath->query('//button[@data-report-submit]')->length===1,'Single responsive submit action, no duplicate mobile toolbar');
portalCheck($xpath->query('//a[contains(@class,"teacher-report-back") and contains(@href,"/portal-guru/dashboard?sesi_id=")]')->length===1,'Focus form has a dashboard back button above its title');
portalCheck($xpath->query('//button[@type="submit" and contains(@formaction,"/arsipkan")]')->length===1,'Archive button uses explicit archive action');
portalCheck(strpos($html,'teacher-report-back')<strpos($html,'<h1>Pengisian Rapor</h1>'),'Back button appears before the report heading');
portalCheck(str_contains(file_get_contents(ROOT_PATH.'/routes/web.php'),"/portal-guru/rapor/{id}/arsipkan"),'Archive action route is registered');
portalCheck($xpath->query('//details[contains(@class,"pengisian-kategori")]/summary')->length===1,'Area has a native accessible collapse trigger');
portalCheck($xpath->query('//details//details[contains(@class,"pengisian-subkategori")]/summary')->length===1,'Subcategory collapses independently inside area');
portalCheck($xpath->query('//details[@open]')->length===0,'Assessment cards start collapsed');
portalCheck($xpath->query('//details//select[@data-report-value and not(@disabled)]')->length===2,'Collapsed values remain enabled and submitted');
portalCheck($xpath->query('//details//textarea')->length===1,'Area collapse includes teacher notes');
